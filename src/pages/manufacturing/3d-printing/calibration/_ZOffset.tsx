...
const ZOffsetDiagram: React.FC<{ value: number }> = ({ value }) => {
  ...
  return (
    <svg width="700" height="550" viewBox="0 0 700 550">
      {/* Hotend body */}
      <rect x="300" y={nozzleY - 80} width="100" height="60" rx="6" fill={colors.darkBlue} stroke={colors.skyBlue + "44"} strokeWidth="2" />
      <text x="350" y={nozzleY - 45} textAnchor="middle" fill={colors.skyBlue} fontSize="12" fontFamily="Space Grotesk">
        HOTEND
      </text>

      {/* Heater block */}
      <rect x="310" y={nozzleY - 20} width="80" height="30" rx="4" fill={calibration.tooHigh + "66"} stroke={calibration.tooHigh + "88"} strokeWidth="1" />

      {/* Nozzle cone */}
      <polygon
        points={`320,${nozzleY + 10} 380,${nozzleY + 10} 360,${nozzleY + 35} 340,${nozzleY + 35}`}
        fill={colors.skyBlue + "cc"}
        stroke={colors.skyBlue}
        strokeWidth="2"
      />
      {/* Nozzle tip */}
      <rect x="343" y={nozzleY + 35} width="14" height="8" rx="2" fill={colors.skyBlue} />
      ...
      {/* Bed surface */}
      <rect x="100" y={bedY} width="500" height="12" rx="4" fill={colors.skyBlue + "44"} />
      <rect x="100" y={bedY + 12} width="500" height="60" rx="4" fill={colors.deepBlue} stroke={colors.skyBlue + "22"} strokeWidth="1" />
      <text x="350" y={bedY + 48} textAnchor="middle" fill={colors.skyBlue + "66"} fontSize="14" fontFamily="Space Grotesk">
        BUILD PLATE
      </text>
      ...
      <g transform="translate(350, 420)">
        ...
        <line x1="-100" y1={40 + layerHeight / 2} x2="100" y2={40 + layerHeight / 2} stroke={colors.skyBlue + "44"} strokeWidth="2" />
      </g>

      {/* Labels for extremes */}
      {value < 0.2 && (
        <text x="350" y={nozzleY - 95} textAnchor="middle" fill={calibration.tooLow} fontSize="18" fontWeight="bold" fontFamily="Space Grotesk">
          Nozzle too far — filament won't stick
        </text>
      )}
      {value > 0.8 && (
        <g>
          <text x="350" y={nozzleY - 95} textAnchor="middle" fill={calibration.tooHigh} fontSize="18" fontWeight="bold" fontFamily="Space Grotesk">
