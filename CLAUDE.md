...
- **Layout (required)**: every page sets the identical root-anchored frontmatter line — `layout: /src/layouts/PageLayout.astro` (resolved from the project root, so pages can move freely without touching it). A page without it renders as bare unstyled HTML. Rendering is data-driven from frontmatter: `cards: true` styles link lists as card tiles (map/MOC pages); `status`, `severity`, `priority`, `category`, `tags` render as badges/chips when present; the root page (`/`) gets the home shell (no breadcrumbs/title header). Title falls back to the URL segment, Obsidian-style; breadcrumbs come from `breadcrumbsFor` in `src/lib/content.ts`.
